    ...
    <div className="min-h-screen bg-slate-900 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-8 animate-fade-in-up">
        <div className="text-center mb-6">
           ...
           <h1 className="text-2xl font-bold text-slate-800">ManutStock Pro</h1>
           <p className="text-slate-500">{isLoginMode ? 'Acesse o sistema' : 'Solicitar acesso'}</p>
        </div>
        ...
        <div className="flex bg-slate-100 p-1 rounded-lg mb-6">
            <button 
                ...
                className={`flex-1 py-2 rounded-md text-sm font-medium transition-all ${isLoginMode ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-500'}`}
            >
                Entrar
            </button>
            <button 
        ...
        {isLoginMode ? (
            ...
            <p className="text-xs text-center text-slate-400 mt-2">Seu cadastro passará por aprovação.</p>
            </form>
        )}
      </div>
    </div>
